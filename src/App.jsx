...
const styleSheet = `
  ...
  .animate-float-delayed { animation: float 12s ease-in-out infinite 2s; }
  .animate-float-slow { animation: float 15s ease-in-out infinite 1s; }
  ...
  .iridescent-text {
    background: linear-gradient(to right, #60a5fa, #c084fc, #fbbf24, #60a5fa);
    background-size: 300% auto;
    -webkit-background-clip: text;
    -webkit-text-fill-color: transparent;
    animation: auroraShift 10s linear infinite;
  }
...
export default function App() {
  ...
  return (
    ...
    >
      ...
      >
        <div className="flex flex-col items-center animate-float w-full max-w-[90vw]">
          <div className="flex items-center justify-center gap-2 md:gap-4 mb-4">
             ...
             <div className="border border-white/50 rounded-full p-1 shadow-[0_0_15px_rgba(255,255,255,0.1)]">
               <div className="border border-white/50 rounded-full w-8 h-8 md:w-10 md:h-10 flex items-center justify-center">
                 <span className="font-serif text-white/80 text-sm md:text-lg">R</span>
               </div>
             </div>
             <div className="h-[1px] w-12 md:w-24 bg-gradient-to-r from-transparent via-white/50 to-transparent"></div>
          </div>
          <h1 className="font-serif text-4xl sm:text-6xl md:text-8xl tracking-wider text-center w-full iridescent-text drop-shadow-[0_0_20px_rgba(255,255,255,0.1)]">
            TopherLoring
          </h1>
      ...
      {/* --- RESPONSIVE INTERACTIVE DOCK (Bottom Anchored) --- */}
      <div className={`absolute bottom-0 left-0 right-0 z-30 transition-all duration-700 ${activeDrawer ? 'opacity-0 pointer-events-none translate-y-12' : 'opacity-100 translate-y-0'}`}>
        ...
        {/* Unified Mobile and Desktop Dock */}
        <div className="max-w-4xl mx-auto flex justify-center items-end gap-4 md:gap-12 pb-12 px-6">
          {Object.values(portfolioData).map((node, i) => {
            ...
            // Layout logic for orbital-feel buttons at the bottom
            return (
              <button 
                key={node.id}
                ...
                className={`group flex flex-col items-center transition-transform duration-500 ${isCenter ? 'translate-y-[-10px] md:translate-y-[-20px]' : ''} animate-float`}
                style={{ 
                  animationDelay: `${i * 0.5}s`,
                  transform: `translateY(${isCenter ? (window.innerWidth >= 768 ? -20 : -10) : 0}px) ${getTransform(20)}`
                }}
              >
      ...
      {/* --- HIDDEN DRAWER (Colored Absolute Masking) --- */}
      <div 
        ...
        >
          ...
          {activeDrawer && (
            <div className="relative z-10 pt-8 pb-24 clear-both animate-in fade-in duration-1000 slide-in-from-right-8">
              ...
              {/* Conditional Rendering: List Layout vs Custom About Layout */}
              {portfolioData[activeDrawer].type === 'list' ? (
                  ...
                  {portfolioData[activeDrawer].sections.map((section, idx) => (
                    <div key={idx} className="relative">
                      <div className="sticky top-0 bg-[#0a0a0c]/90 backdrop-blur-md z-20 py-4 mb-6 md:mb-8 border-b border-white/5">
                        ...
                        {section.description && (
                          <p className="text-gray-500 text-xs md:text-sm mt-2 font-light">{section.description}</p>
                        )}
                      </div>
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-8 md:gap-y-12 relative">
                        {section.items.map((item, itemIdx) => {
                          ...
                          return (
                            <div 
                              key={itemIdx} 
                              className={`group relative p-6 bg-white/[0.02] hover:bg-white/[0.05] border border-white/[0.03] transition-all duration-500 rounded-lg overflow-hidden hover:${portfolioData[activeDrawer].theme.border} ${isOffset ? 'md:mt-12' : ''}`}
                            >
                              ...
                              <div className="relative z-10 flex flex-col h-full">
                                ...
                                <p className="text-xs md:text-sm text-gray-400 leading-relaxed font-light transition-colors duration-500 group-hover:text-gray-300">
                                  {item.desc}
                                </p>
                              </div>
                            </div>
                ...
                /* About / Contact Custom Layout */
                <div className="grid grid-cols-1 lg:grid-cols-5 gap-12 lg:gap-16">
                  
                  {/* Left Column: Info & Details */}
                  <div className="lg:col-span-2 space-y-12">
                    <div className="space-y-6">
                      <div className="w-12 h-12 rounded-full border border-slate-500/30 flex items-center justify-center bg-slate-800/20 mb-6">
                        <User className="w-5 h-5 text-slate-300" />
                      </div>
                      <div>
                        <h3 className="font-serif text-2xl text-gray-100">TopherLoring Industries</h3>
                        <div className="inline-block px-3 py-1 bg-white/5 border border-white/10 rounded-full text-[10px] tracking-widest uppercase text-gray-400 mt-2 mb-4">
                    ...
                    <div className="space-y-6 border-t border-white/5 pt-8">
                      <div className="flex items-start gap-4 group">
                        <div className="w-10 h-10 rounded-full border border-white/5 flex items-center justify-center bg-white/[0.02] group-hover:border-slate-500/50 transition-colors shrink-0">
                          <MapPin className="w-4 h-4 text-gray-500 group-hover:text-slate-300 transition-colors" />
                        </div>
                        <div>
                          <h4 className="text-sm font-serif text-gray-200 mb-1">Headquarters</h4>
                          <p className="text-xs text-gray-500 font-light">Universal City, TX<br/>United States</p>
                        </div>
                      </div>

                      <div className="flex items-start gap-4 group">
                        <div className="w-10 h-10 rounded-full border border-white/5 flex items-center justify-center bg-white/[0.02] group-hover:border-slate-500/50 transition-colors shrink-0">
                          <Mail className="w-4 h-4 text-gray-500 group-hover:text-slate-300 transition-colors" />
                        </div>
                        <div>
                          <h4 className="text-sm font-serif text-gray-200 mb-1">Direct Routing</h4>
                          <a href="mailto:inquiries@topherloring.com" className="text-xs text-gray-500 font-light hover:text-slate-300 transition-colors">
                  ...
                  <div className="lg:col-span-3">
                    <div className="p-8 bg-white/[0.02] border border-white/5 rounded-lg relative overflow-hidden">
                      {/* Subdued watermark for form area */}
                      ...
                      <form onSubmit={handleContactSubmit} className="space-y-6 relative z-10">
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          <div className="space-y-2">
                            <label className="text-xs tracking-widest uppercase text-gray-500">Originator</label>
                            <input 
                              type="text" 
                              required
                              placeholder="Name / Entity"
                              className="w-full bg-black/40 border border-white/10 rounded px-4 py-3 text-sm text-gray-200 placeholder-gray-700 focus:outline-none focus:border-slate-400 focus:bg-black/60 transition-all"
                            />
                          </div>
                          <div className="space-y-2">
                            <label className="text-xs tracking-widest uppercase text-gray-500">Return Address</label>
                            <input 
                              type="email" 
                              required
                        ...
                        <div className="space-y-2">
                          <label className="text-xs tracking-widest uppercase text-gray-500">Vector / Subject</label>
                          <input 
                            type="text" 
                            required
                        ...
                        <div className="space-y-2">
                          <label className="text-xs tracking-widest uppercase text-gray-500">Transmission</label>
                          <textarea 
                            required
                            rows="5"
                            placeholder="Enter transmission..."
                            className="w-full bg-black/40 border border-white/10 rounded px-4 py-3 text-sm text-gray-200 placeholder-gray-700 focus:outline-none focus:border-slate-400 focus:bg-black/60 transition-all resize-none"
                          ></textarea>
                        </div>

                        <button 
                          type="submit"
                          className="w-full md:w-auto px-8 py-3 bg-slate-800 text-slate-200 text-xs tracking-widest uppercase rounded hover:bg-slate-700 hover:text-white transition-colors border border-slate-600/50 flex items-center justify-center gap-3"
                        >
